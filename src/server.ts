...
import { renderInformePdf, type RenderOptions } from './render.js';
...
export function createInformePdfServer(options: ServiceOptions): Server {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.url === '/health' && req.method === 'GET') return json(200, { status: 'ok' });
    if (req.url !== '/render') return json(404, { error: 'Not found' });
    ...
    if (active >= maxConcurrent) return json(503, { error: 'Renderer busy; retry later' });
    active++;
    try {
      let size = 0;
      const chunks: Buffer[] = [];
      for await (const chunk of req) {
        size += chunk.length;
        if (size > maxBody) { json(413, { error: 'Request too large' }); return; }
      ...
      const input = payload.report
        ? { ...buildInformeDocuments(payload.report, payload.content!, payload.template), previewImages: payload.previewImages }
        : { html: payload.html!, coverHtml: payload.coverHtml, continuationHeader: payload.continuationHeader, previewImages: payload.previewImages };
      const result = await renderInformePdf(input, options);
      if (payload.previewImages) return json(200, { pages: result.pages });
      res.writeHead(200, {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="${payload.filename}.pdf"`,
